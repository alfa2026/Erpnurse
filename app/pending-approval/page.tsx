...
import { getPendingUserById } from '@/lib/pending-users'
...
export default function PendingApprovalPage() {
  ...
  const checkStatus = useCallback(async () => {
    ...
    const entry = await getPendingUserById(pid)
    if (!entry) { router.push('/login'); return }

    if (entry.status === 'approved' && entry.role) {
      setStatus('approved')
      // Build full user and save, then redirect
      const u = {
        id: entry.id,
        name: entry.name,
        nameAr: entry.name,
